...
def get_args():
    ...
    parser.add_argument("--tta-actor-lr", type=float, default=3e-4)
    ...
    # parser.add_argument("--tta-actor-lr", type=float, default=0)
    ...
    parser.add_argument("--only-eval-tta", type=bool, default=True)
...
def evaluate(args=get_args()):
    ...
    critic_q1_backbone = MLP_LN_Before_Activation(input_dim=np.prod(args.obs_shape)+args.action_dim, hidden_dims=args.hidden_dims)
    critic_q2_backbone = MLP_LN_Before_Activation(input_dim=np.prod(args.obs_shape)+args.action_dim, hidden_dims=args.hidden_dims)
    critic_v_backbone = MLP_LN_Before_Activation(input_dim=np.prod(args.obs_shape), hidden_dims=args.hidden_dims)
    ...
    # tta_actor_optim = torch.optim.Adam(tta_actor.parameters(), lr=0)
    ...
    print("Critic Q1 optimizer learning rate:", critic_q1_optim.param_groups[0]['lr'])
    print("Critic Q2 optimizer learning rate:", critic_q2_optim.param_groups[0]['lr'])
    print("Critic V optimizer learning rate:", critic_v_optim.param_groups[0]['lr'])
