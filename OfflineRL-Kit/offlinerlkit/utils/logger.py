...
class TensorBoardOutputHandler(KVWriter):
    ...
    def writetensors(self, kvs: Dict) -> None:
        def summary_tensor(k, v):
            kwargs = {'tag': k, 'values': v, 'global_step': self.step}
            self.writer.add_histogram(**kwargs)
...
class Logger(object):
    ...
    def dumptensorboardsclarkvs(self, exclude:Optional[Union[str, Tuple[str, ...]]]=None) -> None:
        # log timestep
        self.logkv(DEFAULT_X_NAME, self._timestep)
        for handler in self._output_handlers:
            if isinstance(handler, TensorBoardOutputHandler):
    ...
    def dumptensorkvs(self, exclude:Optional[Union[str, Tuple[str, ...]]]=None) -> None:
        # log timestep
        self.logkv(DEFAULT_X_NAME, self._timestep)
        for handler in self._output_handlers:
            # if isinstance(handler, KVWriter):
            ...
            #     handler.writekvs(self._name2val)
            if isinstance(handler, TensorBoardOutputHandler):
                handler.writetensors(self._name2val)
        self._name2val.clear()
        self._name2cnt.clear()
