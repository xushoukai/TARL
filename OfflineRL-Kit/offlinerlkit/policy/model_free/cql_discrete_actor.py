...
class CQLPolicyDiscreteActor(SACPolicyDiscrete):
    """
    ...
    def calc_pi_values(
        ...
        action_value = torch.tensor([[-0.9], [-0.7], [-0.5], [-0.3], [-0.1], [0.1], [0.3], [0.5], [0.7], [0.9]], device=act.device)
        action_value = torch.unsqueeze(action_value, dim=0)
        act = torch.squeeze(torch.matmul(act, action_value))
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        action_value = torch.tensor([[-0.9], [-0.7], [-0.5], [-0.3], [-0.1], [0.1], [0.3], [0.5], [0.7], [0.9]], device=a.device)
        action_value = torch.unsqueeze(action_value, dim=0)
        a = torch.squeeze(torch.matmul(a, action_value))
        ...
        if self._max_q_backup:
            ...
            with torch.no_grad():
                ...
                action_value = torch.tensor([[-0.9], [-0.7], [-0.5], [-0.3], [-0.1], [0.1], [0.3], [0.5], [0.7], [0.9]], device=next_actions.device)
                action_value = torch.unsqueeze(action_value, dim=0)
                next_actions = torch.squeeze(torch.matmul(next_actions, action_value))
        ...
        random_ints = torch.randint(10, (batch_size * self._num_repeat_actions, actions.shape[-1]))
        # 使用scatter函数创建one-hot向量
        one_hot_vector = torch.zeros(batch_size * self._num_repeat_actions, actions.shape[-1], 10)
        ...
        # print("random_value: ", random_actions.shape)
