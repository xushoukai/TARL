...
class MCQRDropoutPolicy(SACPolicy):
    """
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        # update actor with RDropout
        a, log_probs, dist = self.actforward_dropout(obss)
        q1a, q2a = self.critic1(obss, a), self.critic2(obss, a)
        repeat_obss = obss.clone()
        repeat_a, repeat_log_probs, repeat_dist = self.actforward_dropout(repeat_obss)
        repeat_q1a, repeat_q2a = self.critic1(repeat_obss, repeat_a), self.critic2(repeat_obss, repeat_a)

        action_dists = torch.distributions.Normal(dist.mean, dist.stddev)
        repeat_action_dists = torch.distributions.Normal(repeat_dist.mean, repeat_dist.stddev)
        p_q_kl = torch.mean(
            torch.distributions.kl.kl_divergence(action_dists, repeat_action_dists))  
        q_p_kl = torch.mean(
            torch.distributions.kl.kl_divergence(repeat_action_dists, action_dists)) 

        actor_loss = - torch.min(q1a, q2a).mean() + self._alpha * log_probs.mean()
        repeat_actor_loss = - torch.min(repeat_q1a, repeat_q2a).mean() + self._alpha * repeat_log_probs.mean()
        
        self.actor_optim.zero_grad()
        (actor_loss + repeat_actor_loss + 0.5 * (p_q_kl + q_p_kl)).backward()
        self.actor_optim.step()

        if self._is_auto_alpha:
            log_probs = log_probs.detach() + self._target_entropy
            repeat_log_probs = repeat_log_probs.detach() + self._target_entropy
            alpha_loss = -(self._log_alpha * log_probs).mean() - (self._log_alpha * repeat_log_probs).mean()
