...
from offlinerlkit.policy import SACPolicyDiscrete


class CQLPolicyDiscreteActor100(SACPolicyDiscrete):
    """
    ...
    def calc_pi_values(
        ...
        act = self.actforward(obs_pi)
        a_value = torch.tensor([[-(1.01 - 0.02 * (i + 1))] for i in range(100)], device=act.device)
        a_value = torch.unsqueeze(a_value, dim=0)
        act = torch.squeeze(torch.matmul(act, a_value))
    ...
    def learn(self, batch: Dict) -> Dict[str, float]:
        ...
        a_value = torch.tensor([[-(1.01 - 0.02 * (i + 1))] for i in range(100)], device=a.device)
        a_value = torch.unsqueeze(a_value, dim=0)
        a = torch.squeeze(torch.matmul(a, a_value))
        ...
        if self._max_q_backup:
            with torch.no_grad():
                ...
                tmp_next_actions = self.actforward(tmp_next_obss)
            ...
            with torch.no_grad():
                ...
                a_value = torch.tensor([[-(1.01 - 0.02 * (i + 1))] for i in range(100)], device=next_actions.device)
                a_value = torch.unsqueeze(a_value, dim=0)
                next_actions = torch.squeeze(torch.matmul(next_actions, a_value))
                next_q = torch.min(
        ...
        # TODO: create random action
        random_ints = torch.randint(100, (batch_size * self._num_repeat_actions, actions.shape[-1]))
        # 使用scatter函数创建one-hot向量
        one_hot_vector = torch.zeros(batch_size * self._num_repeat_actions, actions.shape[-1], 100)
        random_actions = one_hot_vector.scatter_(2, random_ints.unsqueeze(2), 1)
        action_value = torch.tensor([[-(1.01 - 0.02 * (i + 1))] for i in range(100)], device=one_hot_vector.device)
        # action_value = torch.tensor([[-0.9], [-0.7], [-0.5], [-0.3], [-0.1], [0.1], [0.3], [0.5], [0.7], [0.9]], device=one_hot_vector.device)
        action_value = torch.unsqueeze(action_value, dim=0)
        random_actions = torch.squeeze(torch.matmul(random_actions, action_value))
        ...
        for value in [
            ...
            # value.reshape(batch_size, self._num_repeat_actions, 10)
        ...
        result =  {
            ...
            "loss/critic1": critic1_loss.item(),
            "loss/critic2": critic2_loss.item(),
            "loss/conservative_loss1": conservative_loss1.item(),
            "loss/conservative_loss2": conservative_loss2.item(),
        }
