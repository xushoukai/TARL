...
MultiHeadConvFcLayerNormDropoutNetworkType = collections.namedtuple(
    'multi_head_convfc_ln_dropout_dqn_network', ['q_heads', 'unordered_q_heads', 'q_values'])
MultiHeadConvFcLayerNormNetworkType = collections.namedtuple(
    'multi_head_convfc_ln_dqn_network', ['q_heads', 'unordered_q_heads', 'q_values'])
...
QuantileLayerNormNetworkType = collections.namedtuple(
    'qr_ln_dqn_network', ['q_values', 'logits', 'probabilities'])
QuantileConFcLayerNormNetworkType = collections.namedtuple(
    'qr_ConFc_ln_dqn_network', ['q_values', 'logits', 'probabilities'])
QuantileBatchNormNetworkType = collections.namedtuple(
    'qr_bn_dqn_network', ['q_values', 'logits', 'probabilities'])
QuantileConFcLayerNormNeDropouttworkType = collections.namedtuple(
    'qr_ConFc_ln_dropout_dqn_network', ['q_values', 'logits', 'probabilities'])


# QuantileNetwork with layer normalization and dropout
class QuantileConFcLayerNormDropoutNetwork(tf.keras.Model):
  ...
  def __init__(self,
               num_actions: int,
               num_atoms: int,
               name: str = 'quantile_conFc_layernorm_network'):
    ...
    """
    super(QuantileConFcLayerNormDropoutNetwork, self).__init__(name=name)
    ...
    self.dense1 = tf.keras.layers.Dense(
        ...
        kernel_initializer=self._kernel_initializer,
        activation=None)
    self.relu_dense = tf.keras.layers.ReLU()
    self.ln_dense = tf.keras.layers.LayerNormalization(axis=-1)
    self.dropout_dense = tf.keras.layers.Dropout(self.dropout_rate)
  ...
  def call(self, state):
    ...
    net = tf.div(net, 255.)
    # add layer normalization for Conv and Fc
    net = self.dropout1(self.relu1(self.ln1(self.conv1(net))))
    net = self.dropout2(self.relu2(self.ln2(self.conv2(net))))
    net = self.dropout3(self.relu3(self.ln3(self.conv3(net))))
    net = self.flatten(net)
    net = self.dropout_dense(self.relu_dense(self.ln_dense(self.dense1(net))))
    net = self.dense2(net)
    ...
    return QuantileConFcLayerNormNeDropouttworkType(q_values, logits, probabilities)

  
# QuantileNetwork with layer normalization
class QuantileConFcLayerNormNetwork(tf.keras.Model):
  """Keras network for QR-DQN agent.
  ...
  def __init__(self,
    ...
    Args:
      ...
      name: str, used to create scope for network parameters.
    """
    super(QuantileConFcLayerNormNetwork, self).__init__(name=name)
    self.num_actions = num_actions
    self.num_atoms = num_atoms
    self._kernel_initializer = tf.keras.initializers.VarianceScaling(
        scale=1.0 / np.sqrt(3.0), mode='fan_in', distribution='uniform')
  ...
  def call(self, state):
    ...
    net = tf.div(net, 255.)
    # add layer normalization for Conv and Fc
    net = self.relu1(self.ln1(self.conv1(net)))
    net = self.relu2(self.ln2(self.conv2(net)))
    net = self.relu3(self.ln3(self.conv3(net)))
    net = self.flatten(net)
    net = self.relu_dense(self.ln_dense(self.dense1(net)))
    net = self.dense2(net)
    logits = tf.reshape(net, [-1, self.num_actions, self.num_atoms])
    probabilities = tf.keras.activations.softmax(tf.zeros_like(logits))
    q_values = tf.reduce_mean(logits, axis=2)
    return QuantileConFcLayerNormNetworkType(q_values, logits, probabilities)


# QuantileNetwork with layer normalization
class QuantileLayerNormNetwork(tf.keras.Model):
  """Keras network for QR-DQN agent.
  ...
  def __init__(self,
               num_actions: int,
               num_atoms: int,
               name: str = 'quantile_layernorm_network'):
    """Convolutional network used to compute the agent's Q-value distribution.
    ...
    Args:
      ...
      name: str, used to create scope for network parameters.
    """
    super(QuantileLayerNormNetwork, self).__init__(name=name)
    self.num_actions = num_actions
    self.num_atoms = num_atoms
  ...
  def call(self, state):
    ...
    net = self.relu(self.ln(net))
    # net = self.relu(net)
    net = self.dense2(net)
    logits = tf.reshape(net, [-1, self.num_actions, self.num_atoms])
    probabilities = tf.keras.activations.softmax(tf.zeros_like(logits))
    q_values = tf.reduce_mean(logits, axis=2)
    return QuantileLayerNormNetworkType(q_values, logits, probabilities)


# QuantileNetwork With Batch Normalization
class QuantileBatchNormNetwork(tf.keras.Model):
  """Keras network for QR-DQN agent.
  ...
  def __init__(self,
               ...
               num_actions: int,
               num_atoms: int,
               name: str = 'quantile_batchnorm_network'):
    """Convolutional network used to compute the agent's Q-value distribution.
    ...
    Args:
      ...
      name: str, used to create scope for network parameters.
    """
    super(QuantileBatchNormNetwork, self).__init__(name=name)
    ...
    self.dense1 = tf.keras.layers.Dense(
        ...
        kernel_initializer=self._kernel_initializer)
    self.relu = tf.keras.layers.ReLU()
    self.bn = tf.keras.layers.BatchNormalization(axis=-1, trainable=self.trainable)
    self.dense2 = tf.keras.layers.Dense(
        units=num_actions * num_atoms,
        kernel_initializer=self._kernel_initializer,
        activation=None)
  ...
  def call(self, state, training):
    ...
    net = self.conv3(net)
    net = self.flatten(net)
    net = self.relu(self.bn(self.dense1(net), training=training))
    net = self.dense2(net)
    # TODO: what logits means????????
    logits = tf.reshape(net, [-1, self.num_actions, self.num_atoms])
    probabilities = tf.keras.activations.softmax(tf.zeros_like(logits))
    q_values = tf.reduce_mean(logits, axis=2)
    return QuantileBatchNormNetworkType(q_values, logits, probabilities)
...
class MultiHeadQConFcLayerNormDropoutNetwork(tf.keras.Model):
  ...
  def __init__(self,
    ...
    """
    super(MultiHeadQConFcLayerNormDropoutNetwork, self).__init__(name=name)
  ...
  def call(self, state):
    ...
    return MultiHeadConvFcLayerNormDropoutNetworkType(q_heads, unordered_q_heads, q_values)


class MultiHeadQConFcLayerNormNetwork(tf.keras.Model):
  """Multi-head convolutional network to compute multiple Q-value estimates.
  ...
  def __init__(self,
    ...
    """
    super(MultiHeadQConFcLayerNormNetwork, self).__init__(name=name)
    activation_fn = tf.keras.activations.relu
    self.num_actions = num_actions
    self.num_heads = num_heads
    self._transform_strategy = transform_strategy
  ...
  def call(self, state):
    ...
    q_heads, q_values = combine_q_functions(
        unordered_q_heads, self._transform_strategy, **self._kwargs)
    return MultiHeadConvFcLayerNormNetworkType(q_heads, unordered_q_heads, q_values)
