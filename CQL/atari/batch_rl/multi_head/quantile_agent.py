...
class QuantileAgent(rainbow_agent.RainbowAgent):
  ...
  def _create_network(self, name):
    ...
    if callable(self.network):
      ...
      # if Q network use batch normalization
      if self.network == "atari_helpers.QuantileBatchNormNetwork":
        network_class = eval(self.network)
        if self.eval_mode == False:
          network = network_class(False, self.num_actions, self._num_atoms, name=name)
        else:
          network = network_class(True, self.num_actions, self._num_atoms, name=name)
      else:
        network_class = eval(self.network)
        network = network_class(self.num_actions, self._num_atoms, name=name)
    ...
    with tf.control_dependencies([update_priorities_op]):
      if self.summary_writer is not None:
        with tf.variable_scope('Losses'):
          loss_summary = tf.summary.scalar('QuantileLoss', tf.reduce_mean(loss))
          min_q_loss_summary = tf.summary.scalar('minQLoss', tf.reduce_mean(min_q_loss))
          replay_chosen_q_summary = tf.summary.scalar('Q_predictions', tf.reduce_mean(replay_chosen_q))
          merged_train_summaries = tf.compat.v1.summary.merge([loss_summary, min_q_loss_summary, replay_chosen_q_summary])

      min_q_loss = min_q_loss * self.min_q_weight
      return self.optimizer.minimize(tf.reduce_mean(loss) + min_q_loss), loss, merged_train_summaries
