...
class DQNAgent(object):
  ...
  def _build_networks(self):
      ...
      if self.adapted:
        ...
        if self.summary_writer is not None:
          with tf.compat.v1.variable_scope('eval'):
            ...
            self._merged_eval_summaries = tf.compat.v1.summary.merge([eval_q_value, eval_q_entropy])
    ...
    if self.adapted:
      ...
      self._test_before_action_distribution = tf.nn.softmax(self._net_outputs.q_values, axis=-1)
      ...
      # self._test_loss = self._test_kl_divergence + self._test_q_entropy
      self._test_loss = self._test_q_entropy
  ...
  def _select_adapted_action(self):
    ...
    if random.random() <= epsilon:
      ...
      # self.adapted_state = np.arange(0, 84 * 84 * 4).reshape((1, 84, 84, 4)) 
      ...
      if entropy < self.entropy:
        ...
        print("TTA Q value: ", self._sess.run(self._test_net_outputs.q_values, 
                                              {self.adapted_state_ph: self.adapted_state}))
        # print("kl divergence: ", self._sess.run(self._test_kl_divergence, 
        #                               {self.state_ph: self.adapted_state, 
        #                                self.adapted_state_ph: self.adapted_state}))
        # # 新更新模型
  ...
  def _select_action(self):
    ...
    if random.random() <= epsilon:
      ...
      if self.adapted and self.summary_writer is not None:
        summary = self._sess.run(self._merged_eval_summaries, {self.state_ph: self.state})
