...
class DQNAgent(object):
  ...
  def __init__(self,
    ...
    with tf.device(tf_device):
      ...
      # self._test_op = self._build_test_op()
  ...
  def _build_networks(self):
    ...
    if self.adapted:
      self.online_convnet = tent.configure_model(self.online_convnet)
    ...
    if self.network == "atari_helpers.QuantileBatchNormNetwork":
      if self.eval_mode == True:
        ...
        self._q_entropy = tent.softmax_entropy(self._net_outputs.q_values)
        update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
          self.test_op = self.test_optimizer.minimize(tf.reduce_mean(self._q_entropy, axis=0))
  ...
  # def _build_test_op(self):
  #   """Builds a test op.

  #   Returns:
  #     test_op: An op performing one step of testing from environment data.
  #   """
  #   # add bn update operation into graph
  #   if self.network == "atari_helpers.QuantileBatchNormNetwork":
  #     self._net_outputs = self.online_convnet(self.state_ph, training=False)
  ...
  #     update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
  #     with tf.control_dependencies(update_ops):
  #       test_op = self.test_optimizer.minimize(tf.reduce_mean(self._q_entropy, axis=0))
  #   else:
  #     self._net_outputs = self.online_convnet(self.state_ph)
  #     self._q_entropy = tent.softmax_entropy(self._net_outputs.q_values)
  #     test_op = self.test_optimizer.minimize(tf.reduce_mean(self._q_entropy, axis=0))
  ...
  def _select_action(self):
    ...
    if random.random() <= epsilon:
      ...
      if self.adapted and self.eval_mode == True:
        if self.test_step % self.update_period == 0:
          self._sess.run(self.test_op, {self.state_ph: self.state})

        if self.test_step % self.target_update_period == 0:
