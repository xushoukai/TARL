...
@gin.configurable(denylist=['variables'])
def transform_variable_names(variables):
  """Maps old variable names to the new ones.
  ...
  name_map = []
  for var in variables:
    new_name = var.op.name.replace('beta:0', 'beta')
    new_name = var.op.name.replace('gamma:0', 'gamma')
    new_weights = tf.Variable(var.initialized_value(), name=new_name)

    # var.op.name.replace('beta:0', 'beta')
    # var.op.name.replace('gamma:0', 'gamma')
    # new_name = new_name.replace('kernel', 'weights')
    name_map.append(new_weights)
  return name_map


# NatureDQNNetwork with layerNorms
class NatureDQNLayerNormNetwork(tf.keras.Model):
  ...
  def __init__(self, num_actions, name=None):
    ...
    """
    super(NatureDQNLayerNormNetwork, self).__init__(name=name)
    ...
    self.dense1 = tf.keras.layers.Dense(512, name='fully_connected')
  ...
  def call(self, state):
    ...
    x = self.flatten(x)
    x = self.relu_dense(self.ln_dense(self.dense1(x)))
